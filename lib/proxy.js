...
var jsLogger = require('js-logger').logger;
var logger = jsLogger.getLogger('Proxy');
...
module.exports = function(options) {
  ...
  return function(req, res) {
    ...
    if(params.url.indexOf('.visualforce.com') != -1) {
        params.followAllRedirects = true; // instanceless url will require a rediriect so always allow it
        params.followRedirect = function(response) {
    ...
    req.pipe(request(params))
      ...
      .on('redirect', function() {
        ...
        logger.info('-------------- end redirect -----------------');
        if(params.url.indexOf('.visualforce.com') != -1)
          // Instanceless URLs (ones that contain visualforce.com) will redirect to the domain specific one,
          // but request.js strips off authorization if host name changes - https://github.com/bagder/curl/blob/6beb0eee/lib/http.c#L710.
          // So add lets add the authroization back here as the host change name is expected
